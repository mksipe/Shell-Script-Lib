...
# Change Port
sed -i "s/#Port.*/Port 62111/g" /etc/ssh/sshd_config
# Protocol 2
...
sed -i "s/^#X11Forwarding.*/#X11Forwarding no/g" /etc/ssh/sshd_config
# Set Grace Time to two minutes
sed -i "s/^LoginGraceTime.*/LoginGraceTime 120" /etc/ssh/sshd_config
sed -i "s/^#LoginGraceTime.*/LoginGraceTime 120" /etc/ssh/sshd_config
# Make sure that passwords are required
sed -i "s/^PasswordAuthentication.*/PasswordAuthentication yes" /etc/ssh/sshd_config
sed -i "s/^PasswordAuthentication.*/PasswordAuthentication yes" /etc/ssh/sshd_config
# Ensuring that PAM is enabled during password authentication
sed -i "s/^UsePAM.*/UsePAM yes" /etc/ssh/sshd_config
sed -i "s/^#UsePAM.*/UsePAM yes" /etc/ssh/sshd_config
# making sure that privilage is separated
sed -i "s/^UsePrivilageSeparation.*/UsePrivilageSeparation yes"
sed -i "s/^#UsePrivilageSeparation.*/UsePrivilageSeparation yes"
